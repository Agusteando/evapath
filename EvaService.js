...
class EvaService {
  ...
  async post(apiPath, bodyData, useApi = false, headers = {}) {
    let attempts = 0;
    const maxAttempts = 3;
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        if (this._reconnectPromise) {
          this._log(`POST paused. Waiting for active EVA reconnect to finish...`);
          await this._reconnectPromise;
        }
        ...
        this._log(`POST ${url} (attempt ${attempts})`);
        console.log(`[EvaService DEBUG] Dispatching Node HTTP POST to: ${url}`);
        ...
        const data = await new Promise((resolve, reject) => {
          ...
          req.setTimeout(45000, () => {
            req.destroy();
            console.error(`[EvaService DEBUG] HTTP POST request to ${url} timed out.`);
            reject(new Error(`HTTP POST Timeout after 45000ms for ${url}`));
          });
        ...
        console.log(`[EvaService DEBUG] Node HTTP POST success for: ${url} (payload size: ${JSON.stringify(data).length})`);
        return data; 

      } catch (err) {
        const msg = err.message || String(err);
        console.warn(`[EvaService DEBUG] POST threw an error: ${msg}`);
        
        const isAuthError = msg.includes("HTTP 401") || msg.includes("Authorization has been denied");
        const isConnectionError = msg.includes("Target closed") || msg.includes("Session closed") || msg.includes("Puppeteer page is closed") || msg.includes("No access token") || msg.includes("context error");
        
        if ((isAuthError || isConnectionError) && attempts < maxAttempts) {
          this._log(`POST recoverable error (${isAuthError ? "AUTH_ERROR" : "CONN_ERROR"}) on attempt ${attempts}. Triggering auto-reconnect sequence...`);
          try {
            await this.reconnect(tokenUsed);
          } catch (reconnectErr) {
            this._log("Auto-reconnect failed inside POST catch block: " + (reconnectErr.message || reconnectErr));
            await delay(2000); 
          }
          continue;
        }
        
        this._log(`POST critical error on attempt ${attempts}: ${msg}`);
