...
export async function GET(req) {
  ...
  try {
    ...
    const [cand] = await pathDB.query(
      `SELECT id,email,CONCAT_WS(' ',nombres,apellidopaterno,apellidomaterno) AS nombre FROM candidatos WHERE email LIKE ? OR CONCAT_WS(' ',nombres,apellidopaterno,apellidomaterno) LIKE ? LIMIT 20`,
      [`%${term}%`, `%${term}%`],
    );
    ...
    if (exclude.length)
      filtered = filtered.filter((c) => !exclude.includes(String(c.id)));
