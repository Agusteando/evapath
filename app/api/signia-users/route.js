  ...
  try {
    ...
    if (eva.ready) {
      try {
        ...
        console.warn("[signia-users] EVA getUsers failed:", e.message);
      }
    }
    const evaMap = Object.fromEntries(
      evaUsers.map((u) => [normalizeEmail(u.correo), u]),
    );

    const emails = sigAll.map((u) => u.email);
    let cand = [];
    if (emails.length) {
      [cand] = await pathDB.query(
        `SELECT id,email FROM candidatos WHERE email IN (${emails.map(() => "?").join(",")})`,
        emails,
      );
    }
    const candIdByEmail = Object.fromEntries(
      cand.map((c) => [normalizeEmail(c.email), c.id]),
    );
    ...
    const emailById = Object.fromEntries(
      cand.map((c) => [c.id, normalizeEmail(c.email)]),
    );
    pruebas.forEach((p) => {
      const mail = emailById[p.candidato_id],
        lab = label(p.catalogo_id);
      const url = pdfURL(p.candidato_id, p.pid, p.code, p.catalogo_id);
      if (!linkByEmail[mail]) linkByEmail[mail] = [];
      if (!linkByEmail[mail].find((x) => x.label === lab))
        linkByEmail[mail].push({ label: lab, url });
    });

    sigAll.forEach((u) => {
      const emailKey = normalizeEmail(u.email);
      const eva = evaMap[emailKey];
      u.evaMatch = !!eva;
      u.evaName = eva?.nombre || "";
      u.evaCID = eva?.CID || null;
      u.evaEstado = eva?.estado || null;
      u.namesMatchEva = eva ? u.name === eva.nombre : false;
      u.pathMatch = !!candIdByEmail[emailKey];
      u.pathLinks = linkByEmail[emailKey] || [];
      u.fechaIngresoISO = u.fechaIngreso
        ? new Date(u.fechaIngreso).toISOString()
        : null;
    });
    ...
    const evaPct = sigAll.length
      ...
      : "0.0";
    const pathPct = sigAll.length
      ? ((pathMat / sigAll.length) * 100).toFixed(1)
      : "0.0";
    ...
    const pag = (a, p, ps) => {
      const tot = a.length,
        last = Math.max(1, Math.ceil(tot / ps));
      p = Math.max(1, Math.min(p, last));
      return {
        items: a.slice((p - 1) * ps, (p - 1) * ps + ps),
        page: p,
        pageSize: ps,
        total: tot,
        lastPage: last,
    ...
    return NextResponse.json({
      ...
      signiaStats: {
        evaMatched: evaMat,
        evaUnmatched: sigAll.length - evaMat,
        evaPct,
        pathMatched: pathMat,
        pathUnmatched: sigAll.length - pathMat,
        pathPct,
        total: sigAll.length,
      },
      page,
