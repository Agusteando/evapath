...
export default function AutoSimilarity({
  ...
  const usersToMatch = useMemo(() => {
    ...
    return list;
  }, [signiaUsers, searchTerm, skippedIds]);
  ...
  const handleSkip = useCallback(() => {
    ...
    setSkippedIds((prev) => new Set([...prev, currentSignia.id]));
  }, [currentSignia]);
  ...
  const handleAssociate = useCallback(
    async (type, match) => {
      ...
      try {
        ...
        // The parent state update removes completed users from this queue.
      } catch (err) {
        console.error("AutoSimilarity Associate Error:", err);
        setActionError(err?.message || "Error al asociar. Revisa la consola.");
      } finally {
        setProcessing(false);
      }
    },
  ...
  useEffect(() => {
    const handleKey = (e) => {
      ...
      if (key === "e") {
        e.preventDefault();
        if (evaMatches[0]) handleAssociate("eva", evaMatches[0]);
        return;
      }
      if (key === "p") {
        e.preventDefault();
        if (pathMatches[0]) handleAssociate("path", pathMatches[0]);
        return;
      }
      ...
      <div className="flex flex-col items-center justify-center py-24 animate-fade-in bg-white rounded-xl border border-slate-200 shadow-sm mx-auto max-w-2xl">
        ...
        <p className="text-slate-600 mb-8 text-center max-w-md">
          {searchTerm
            ...
            : "Todos los usuarios visibles tienen sus enlaces de EVA y PATH completados."}
        </p>
    ...
    <div className="w-full max-w-7xl mx-auto mt-2 pb-10">
      ...
      <div className="text-center mb-4">
        <div className="inline-flex items-center gap-3 bg-slate-50 px-4 py-1.5 rounded-full border border-slate-200 text-xs text-slate-500 shadow-sm font-mono">
          <span>[E] Eva</span> •<span>[P] Path</span> •<span>[S] Saltar</span> •
          <span>[1-3] Eva#</span> •<span>[4-6] Path#</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-4 space-y-3">
          <div className="flex items-center gap-2 mb-2 pb-2 border-b border-indigo-100">
            ...
            <h3 className="font-bold text-indigo-900 uppercase text-sm">
              Sugerencias EVA
            </h3>
          </div>
                ...
                <div className="pl-4">
                  ...
                  <div className="flex gap-1 mb-2">
                    ...
                    {(m.source.pathLinks || []).some(
                      ...
                      <span className="text-[9px] bg-pink-50 text-pink-600 border border-pink-200 px-1 rounded font-bold">
                        MMPI
                      </span>
                    )}
                  </div>
